...
public class CsvWriteRead {
    ...
    public static void main(String[] args) {
        ...
        saveArrayToFile(data, "Lesson_6.csv"); // сохраняем в файл
        data = loadArrayFromFile("Lesson_6.csv"); // читаем из файла в
        data.toStringAppData();
    }

    private static void saveArrayToFile(AppData data, String path) {
        try {
            FileWriter writer = new FileWriter("Lesson_6.csv");
    ...
    private static AppData loadArrayFromFile(String path) {
        AppData data = new AppData();
        try (BufferedReader reader = new BufferedReader(new FileReader("Lesson_6.csv"))) {
